...
nn_name = 'classify_image_graph_def.pb'
...
class NodeLookup(object):
  ...
  def __init__(self,
    ...
    if not label_lookup_path:
      label_lookup_path = os.path.join(model_dir, label_path)
    if not uid_lookup_path:
      uid_lookup_path = os.path.join(model_dir, uid_path)
...
def create_graph():
  ...
  with tf.gfile.FastGFile(os.path.join(model_dir, nn_name), 'rb') as f:
...
def run_inference_on_image(image):
  ...
  with tf.Session() as sess:
    ...
    for node_id in top_k:
      ...
      ret.append({"item" : human_string, "score" : float(score)})
    return ret
        
def classify(img):
    return run_inference_on_image(img)
